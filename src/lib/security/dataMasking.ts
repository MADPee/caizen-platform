...
export const maskRegistrationNumber = (
  ...
  const masked =
    registration.substring(0, visibleStart) +
 ...
 */
export const maskVIN = (vin: string, options: MaskingOptions = {}): string => {
 ...
 */
export const formatForDataExport = (
    ...
    const recursiveMask = (
      ...
      Object.keys(obj).forEach((key) => {
        if (typeof obj[key] === "string") {
          ...
          obj[key] = recursiveMask(obj[key] as Record<string, unknown>);
        }
      });
...
export const DEV_HELPERS = {
  generateTestRegistration: () =>
    `TEST${Math.random().toString(36).substr(2, 3).toUpperCase()}`,
  generateTestVIN: () =>
    "WBA" + Math.random().toString(36).substr(2, 14).toUpperCase(),
  validateMasking: (original: string, masked: string) => {
    ...
    return (
      maskedChars.includes("*") && originalChars.length === maskedChars.length
    );
  },
};
