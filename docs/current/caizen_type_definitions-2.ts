...
export interface WeatherCondition {
  ...
  readonly conditions: 'sunny' | 'cloudy' | 'rainy' | 'snowy' | 'foggy' | 'windy';
...
export interface VerificationBadge {
  readonly type: 'service_history' | 'inspection_passed' | 'low_mileage' | 'one_owner' | 'accident_free';
  readonly verified: boolean;
  readonly verifiedAt?: Date;
  readonly verifiedBy?: string; // Tredjepartsverifierare
...
export interface VehicleGroup {
  readonly id: string;
  readonly name: string; // "BMW E46 Ägare"
  readonly description: string;
  readonly make: string;
  readonly model?: string;
  ...
  readonly moderators: string[]; // User IDs
...
export interface CommunityComment {
  ...
  readonly authorId: string;
  readonly content: string;
  readonly parentCommentId?: string; // För nested comments
  readonly likeCount: number;
  readonly moderationStatus: ModerationStatus;
  readonly createdAt: Date;
  readonly updatedAt: Date;
...
export interface DocumentReference {
  ...
  readonly url: string; // Säker, signerad URL
...
export type PrivacyLevel = 
  | 'public'     // Alla kan se
  | 'community'  // Endast community-medlemmar
  | 'verified'   // Endast verifierade användare
  | 'private';   // Endast ägaren
